...
const CartDrawer = ({ isOpen, onClose }) => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/events/checkout`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          cartItems,
          customerEmail: 'guest@example.com' // Replace with your actual auth state email if available
        })
      });

      const data = await response.json();
      if (data.url) {
        window.location.href = data.url; // Dynamic redirect straight to secure Stripe Checkout
      } else {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 1000, display: 'flex' }}>
      ...
      <div 
        onClick={onClose}
        style={{ width: '100%', height: '100%', backgroundColor: 'rgba(0, 0, 0, 0.4)', backdropFilter: 'blur(2px)', transition: 'opacity 0.3s' }}
      />
      ...
      <div style={{ position: 'absolute', right: 0, top: 0, width: '100%', maxWidth: '450px', height: '100%', backgroundColor: '#fff', boxShadow: '-4px 0 25px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column', padding: '30px' }}>
        ...
        <div style={{ flexGrow: 1, overflowY: 'auto', marginBottom: '20px' }}>
          {cartItems.length === 0 ? (
            <div style={{ textAlignment: 'center', textAlign: 'center', color: '#777', marginTop: '40px' }}>
        ...
        {cartItems.length > 0 && (
          <div style={{ borderTop: '1px solid #f0f0f0', paddingTop: '20px' }}>
            ...
            >
              {loading ? 'Routing to Checkout...' : 'Secure Checkout via Stripe'}
            </button>
          </div>
        )}
